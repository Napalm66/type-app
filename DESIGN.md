...
Replaced the Gantt-style bar chart with a lineage tree: each of the 13 classifications is a box positioned by its real start/end years within its lane, connected to its documented predecessor/successor by an elbowed line with an accent dot (derived from each classification's own `closesBecause` field in `js/data.js`, not invented). Era bands (Romanesque through Digital & Variable Age) run the full chart height behind the lanes rather than sitting as a thin strip above them.

- **Equal-width era bands, permanently** — every era column takes the same share of the chart width regardless of real duration. This was tried the other way (a click-to-open "fisheye" that gave the clicked era a larger share and compressed the rest proportionally) and reverted: the interaction added complexity without earning it, and a duration-accurate ruler underneath an equal-width layout is inherently misleading (100-year gaps rendering at wildly different pixel widths depending which era they fall in). Each node already states its own start–end years as text, so accurate duration doesn't depend on band width.
- **Single-hue tint/shade ramp, not a rainbow or chromatic arc** — era bands are the app's own ink teal at a chronologically deepening alpha (0.16 at Romanesque to 0.64 at Digital & Variable Age), read from the live `--ink` custom property so it follows the light/dark theme automatically rather than a hardcoded hex. This superseded an earlier two-color (teal-to-coral) arc and, before that, a per-era rainbow — both read as more "designed" than the content justified.
- **Vertical photo strips, not one watermark icon** — each era band is filled with equal-width vertical strips (more strips the wider the column) that fade linearly from full opacity at the top to transparent at the base, dissolving into the era's own color wash. Romanesque and Gothic have real black-and-white photos (`images/eras/`); other eras fall back to a placeholder tone + generic image-frame icon until photos are supplied. Real photos: a mix of a person (scribe/printer at work), an object/tool, a type-specimen close-up, and one architectural shot — not four buildings, which read as flat.
- **Minimum-width padding anchors to each node's true end date, capped at its own era's boundary** — a node whose real duration is too short to fit its label needs padding beyond its true span; that padding extends backward from the node's real end year (so it lines up flush with any sibling elsewhere that closes the same year) but is hard-clamped to never cross behind the left edge of the era its start year falls in (so a node never visually appears to begin in the wrong era). When the two constraints conflict and neither can fully win, the era-boundary clamp takes priority over flush-alignment — historical placement over cosmetic tidiness.
- **Node names wrap to two lines before shrinking or overflowing** — same treatment era-band labels already had, extended to classification names once minimum-width padding alone can't fit them (breaks at the best space or hyphen, e.g. "NEO-" / "GROTESQUE"). Two genuinely overlapping classifications (Venetian Oldstyle running to 1500, Garalde Oldstyle starting 1495) get separate lanes rather than being forced flush in one shared lane, so the overlap reads as a branch connector instead of an artificial hand-off.
- **Rollover tooltips carry the same info as the old chart** — hovering an era band or a classification node shows the same description/closesBecause text the Gantt version's tooltips did; clicking a node still opens the full detail view (`onOpenDetail`), unchanged from the bar-chart version.
